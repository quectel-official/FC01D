...
static int mqtt_demo_start_connect(char *host , char * type)
{
	...
	if(strcmp(type, "mqtts") == 0)
	{
		ssl_mqtt_client_api_register( &_mqttc_ses.netport );
		if(mqtt_net_connect(&_mqttc_ses,link_name,8390) != MQTT_OK)
		{
			goto _exit;
		}
	}
...
static void mqtt_test_start(int argc, char **argv)
{
  ...
  //  if(mqtt_demo_start_connect("192.168.19.24") != 0)
    ...
    if(mqtt_demo_start_connect(argv[2],argv[1]) != 0)
	{
		return;
    }
...
static void mqtt_demo_command(char *pcWriteBuffer, int xWriteBufferLen, int argc, char **argv)
{
	...
	if(argc >= 2)
	{
		...
		else if(strcmp(argv[1], "mqtt") == 0 || strcmp(argv[1], "mqtts") == 0)
		{
