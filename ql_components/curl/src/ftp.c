...
#ifdef QUECTEL_CURL_SUPPORT
	//防止此时服务器关闭,ftp会循环调用do more(收到了rst或fin后select不再等待1S) 导致不能喂狗
	int ret = lwip_getTcpState(conn->bits.tcpconnect[FIRSTSOCKET]);
	if ((ret > ESTABLISHED || ret < CLOSED))
	{
		DEBUGF(infof(data, "first socket closed\n"));
		return CURLE_ABORTED_BY_CALLBACK;
	}
#endif
