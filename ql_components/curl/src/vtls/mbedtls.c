...
#ifdef QUECTEL_CURL_SUPPORT
#include "ql_ssl.h"
#include "quec_log.h"

#define ssl_print(msg, ...)     custom_log("[QL_SSL]", msg, ##__VA_ARGS__)
#endif
...
#if defined(QUECTEL_CURL_SUPPORT) && defined(QL_SSL_TLS_CURL_SUPPORT)

extern ql_ssl_config* ql_ftpsetup_ssl_cfg_get(int ctxid);

/* Ignore CA check items */
static int ql_ssl_ignore_ca_check_item(void *data, mbedtls_x509_crt *crt, int depth, uint32_t *flags)
{
    char buf[1024] = {0};
    uint32_t ssl_ctx_ignore_certitem_cfg = *((uint32_t *)data);
    ...
    if ( (0 == ssl_ctx_ignore_certitem_cfg) ||
         (0 ==  *flags) )
    {
       return( 0 );
    }
    ...
    uint32_t cert_item[QL_SSL_TLS_CERT_ITEM_NUM] = {MBEDTLS_X509_BADCERT_EXPIRED, MBEDTLS_X509_BADCERT_REVOKED  , MBEDTLS_X509_BADCERT_CN_MISMATCH  , MBEDTLS_X509_BADCERT_NOT_TRUSTED , MBEDTLS_X509_BADCRL_NOT_TRUSTED,
                                                    MBEDTLS_X509_BADCRL_EXPIRED , MBEDTLS_X509_BADCERT_MISSING  , MBEDTLS_X509_BADCERT_SKIP_VERIFY  , MBEDTLS_X509_BADCERT_OTHER       , MBEDTLS_X509_BADCERT_FUTURE    ,
                                                    MBEDTLS_X509_BADCRL_FUTURE  , MBEDTLS_X509_BADCERT_KEY_USAGE, MBEDTLS_X509_BADCERT_EXT_KEY_USAGE, MBEDTLS_X509_BADCERT_NS_CERT_TYPE, MBEDTLS_X509_BADCERT_BAD_MD    ,
                                                    MBEDTLS_X509_BADCERT_BAD_PK , MBEDTLS_X509_BADCERT_BAD_KEY  , MBEDTLS_X509_BADCRL_BAD_MD        , MBEDTLS_X509_BADCRL_BAD_PK       , MBEDTLS_X509_BADCRL_BAD_KEY    };

    for (int i=0; i < QL_SSL_TLS_CERT_ITEM_NUM; i++)
    {
                   ...
                   int sockindex)
{
  ...
  struct ssl_backend_data *backend = connssl->backend;
#if defined(QUECTEL_CURL_SUPPORT) && defined(QL_SSL_TLS_CURL_SUPPORT)
  ql_ssl_config *conf = ql_ftpsetup_ssl_cfg_get(conn->sslcfgID);
  ...
  mbedtls_ssl_config_defaults(&(backend->config),
  ...
  if (conf->auth_mode == QL_SSL_VERIFY_NULL)
  {
  ...
  else
  {
      ...
      while (kk < QL_MAX_CA_CERT_CNT)
      {
          if (conf->ca_cert_path[kk] != NULL)
          {
              ret = mbedtls_x509_crt_parse_file(&backend->cacert, (char *)conf->ca_cert_path[kk]);
              if (ret < 0)
              {
#ifdef MBEDTLS_ERROR_C
                  mbedtls_strerror(ret, errorbuf, sizeof(errorbuf));
#endif /* MBEDTLS_ERROR_C */
                  failf(data, "Error reading ca cert path %s - mbedTLS: (-0x%04X) %s",(char *)conf->ca_cert_path[kk], -ret, errorbuf);
  ...
  if (conf->auth_mode == QL_SSL_VERIFY_CLIENT_SERVER)
  {
      if (conf->own_cert_path == NULL || conf->own_key_path == NULL)
      {
          return CURLE_SSL_CACERT_BADFILE;
      }

      /* Load the client certificate */
      ret = mbedtls_x509_crt_parse_file(&backend->clicert, (char *)conf->own_cert_path);
      if (ret != 0)
      {
#ifdef MBEDTLS_ERROR_C
          mbedtls_strerror(ret, errorbuf, sizeof(errorbuf));
          ...
          failf(data, "Error reading client cert file %s - mbedTLS: (-0x%04X) %s",
                (char *)conf->own_cert_path, -ret, errorbuf);

          return CURLE_SSL_CERTPROBLEM;
      }

      /* Load the client private key */
      ret = mbedtls_pk_parse_keyfile(&backend->pk, (char *)conf->own_key_path, conf->own_key_pwd);
      if (ret != 0)
      {
#ifdef MBEDTLS_ERROR_C
          mbedtls_strerror(ret, errorbuf, sizeof(errorbuf));
          ...
          failf(data, "Error reading private key %s - mbedTLS: (-0x%04X) %s",
                (char *)conf->own_key_path, -ret, errorbuf);
  ...
  switch (conf->ssl_version)
  {
    ...
    case QL_SSL_VERSION_1:
    {
  	  mbedtls_ssl_conf_min_version(&backend->config, MBEDTLS_SSL_MAJOR_VERSION_3, MBEDTLS_SSL_MINOR_VERSION_1);
  	  mbedtls_ssl_conf_max_version(&backend->config, MBEDTLS_SSL_MAJOR_VERSION_3, MBEDTLS_SSL_MINOR_VERSION_1);
    }
    break;
  
    case QL_SSL_VERSION_2:
    {
  	  mbedtls_ssl_conf_min_version(&backend->config, MBEDTLS_SSL_MAJOR_VERSION_3, MBEDTLS_SSL_MINOR_VERSION_2);
  	  mbedtls_ssl_conf_max_version(&backend->config, MBEDTLS_SSL_MAJOR_VERSION_3, MBEDTLS_SSL_MINOR_VERSION_2);
    }
    break;
  
    case QL_SSL_VERSION_3:
    {
  	  mbedtls_ssl_conf_min_version(&backend->config, MBEDTLS_SSL_MAJOR_VERSION_3, MBEDTLS_SSL_MINOR_VERSION_3);
  	  mbedtls_ssl_conf_max_version(&backend->config, MBEDTLS_SSL_MAJOR_VERSION_3, MBEDTLS_SSL_MINOR_VERSION_3);
    }
    break;
  
    case QL_SSL_VERSION_ALL:
    {
  	  mbedtls_ssl_conf_min_version(&backend->config, MBEDTLS_SSL_MAJOR_VERSION_3, MBEDTLS_SSL_MINOR_VERSION_0);
  	  mbedtls_ssl_conf_max_version(&backend->config, MBEDTLS_SSL_MAJOR_VERSION_3, MBEDTLS_SSL_MINOR_VERSION_3);
  }
  ...
  mbedtls_ssl_conf_handshake_timeout(&backend->config, conf->ssl_negotiate_timeout, conf->ssl_negotiate_timeout);
  ...
  quec_mbedtls_ssl_set_ignore_cert_flag(&(backend->config), conf->ignore_invalid_certsign);
  mbedtls_ssl_conf_verify(&(backend->config), ql_ssl_ignore_ca_check_item, &(conf->ignore_certitem));
