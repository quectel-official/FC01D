...
CURLcode Curl_get_upload_buffer(struct Curl_easy *data)
{
  if(!data->state.ulbuf) {
    ...
    if(!data->state.ulbuf)
      {
        os_printf("ulbuf malloc fail,%d, %d", data->set.upload_buffer_size, xPortGetFreeHeapSize());
  ...
  if(!result) {
    ...
    if(data->state.wildcardmatch) {
      ...
      if(wc->state < CURLWC_INIT) {
        ...
        if(result)
        {
          os_printf("wildcard malloc fail");
          return CURLE_OUT_OF_MEMORY;
        }
      }
    }
