...
int mbedtls_client_context(MbedTLSSession *session)
{
    ...
    ret = mbedtls_x509_crt_parse( &session->mbedX509User, (const unsigned char *) mbedtls_user_certificate,
                          mbedtls_user_certificate_len );
    if( ret != 0 )
    {
    ...
    ret =  mbedtls_pk_parse_key( &session->pkey, (const unsigned char *) mbedtls_pkey_certificate,
                          mbedtls_pkey_certificate_len, NULL, 0 );
    if( ret != 0 )
    {
	...
	mfl_code = MBEDTLS_SSL_MAX_FRAG_LEN_4096;//MBEDTLS_SSL_MAX_FRAG_LEN_1024;///MBEDTLS_SSL_MAX_FRAG_LEN_512;
...
#if CFG_USE_CA_CERTIFICATE
    mbedtls_ssl_conf_own_cert(&session->conf,   &session->mbedX509User, &session->pkey);
#endif
