...
static int ssl_handshake_init( mbedtls_ssl_context *ssl )
{
    ...
    {
        TRACE_ERR_PRINTF("alloc() of ssl sub-contexts failed\r\n");
...
int __wrap_mbedtls_ssl_read(mbedtls_ssl_context *ssl, unsigned char *buf, size_t len)
{
    ...
    TRACE_PRINTF("add mbedtls RX buffer\r\n");
    ret = esp_mbedtls_add_rx_buffer(ssl);
    if (ret == MBEDTLS_ERR_SSL_CONN_EOF) {
        TRACE_ERR_PRINTF("mbedtls_ssl_read fail, the connection indicated an EOF\r\n");
        return 0;
    } else if (ret < 0) {
        if(MBEDTLS_ERR_SSL_TIMEOUT != ret && MBEDTLS_ERR_SSL_WANT_READ != ret)
        {
            TRACE_ERR_PRINTF("mbedtls_ssl_read fail, error=%d\r\n", -ret);
        }
        return ret;
    }
    TRACE_PRINTF( "end\r\n");
